...
from typing import Tuple, Dict, Type, Callable, Any, Union, Optional, List
...
def unpack(env_info: BrainInfo, agents: List[str]) -> Tuple[Dict[str, np.ndarray],
                                                            Dict[str, float],
                                                            Dict[str, bool],
                                                            Dict]:
    ...
    for i, agent in enumerate(agents):
        state[agent] = env_info.vector_observations[i].astype(np.float32)
        reward[agent] = env_info.rewards[i]
        done[agent] = env_info.local_done[i]

    return state, reward, done, info


def pack(actions: Dict[str, np.ndarray]) -> np.ndarray:
    return np.stack([action for _, action in actions.items()])
...
def get_episode_rewards(batch: DataBatch) -> np.ndarray:
    ...
    # TODO: fix this
    ep_lens = get_episode_lens(batch['dones']["Agent0"])

    ep_rewards0 = np.array([torch.sum(rewards) for rewards in torch.split(batch['rewards']['Agent0'], ep_lens)])
    ep_rewards1 = np.array([torch.sum(rewards) for rewards in torch.split(batch['rewards']['Agent1'], ep_lens)])

    return np.maximum(ep_rewards0, ep_rewards1)
